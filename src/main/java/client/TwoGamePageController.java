...
public class TwoGamePageController {
    ...
    void onHit() {
        ...
        if (total == 21) {
            ...
            showHand(engine.getComputadoras().get(0), hboxPlayer2);
            ...
            lblPlayer2.setText("Player 2 " + result + " with " + engine.getComputadoras().get(0).TotalSum() + " points");
    ...
    void onStand() {
        ...
        if (!engine.getComputadoras().isEmpty()) {
            String result2 = GameEngine.EvalHand(engine.getComputadoras().get(0), engine.getCasa());
            showHand(engine.getComputadoras().get(0), hboxPlayer2);
            lblPlayer2.setText("Player 2 " + result2 + " with " + engine.getComputadoras().get(0).TotalSum() + " points");
        }
    }
    ...
    private void startPlaying() {
        ...
        engine.excecuteGame(2);

        showHand(engine.getJugador(), hboxPlayer);

        if (!engine.getComputadoras().isEmpty()) {
            hiddenHand(engine.getComputadoras().get(0), hboxPlayer2);
        }
        ...
        if (engine.gameShouldEndImmediately2p()) {
            showHand(engine.getCasa(), hboxDealer);
            showHand(engine.getComputadoras().get(0), hboxPlayer2);
            String result = GameEngine.EvalHand(engine.getJugador(), engine.getCasa());
            ...
            if (engine.getComputadoras().get(0).IsBlackjack()) {
                lblPlayer2.setText("Blackjack!");
            } else {
                lblPlayer2.setText("Player 2 " + result + " with " + engine.getComputadoras().get(0).TotalSum() + " points");
            }
